...
class UsersModule extends AbstractApiModule {
  ...
  async init () {
    ...
    await this.initUsergroupRoutes(server, auth)
   ...
   */
  async initUsergroupRoutes (server, auth) {
    const rootDir = fileURLToPath(new URL('../usergroups', import.meta.url))
    const defaults = fileURLToPath(import.meta.resolve('adapt-authoring-api/default-routes.json'))
    const config = await loadRouteConfig(rootDir, this, {
      schema: 'apiroutes',
      handlerAliases: { serveSchema: this.serveSchema.bind(this) },
      defaults
    })
    if (!config) return
    ...
    const replacements = {
      '${scope}': config.permissionsScope || config.root,
      '${schemaName}': 'usergroup',
      '${collectionName}': 'usergroups'
    }
    /* eslint-enable no-template-curly-in-string */
    const routes = config.routes.map(r => this.replacePlaceholders(r, replacements))

    this.usergroupsRouter = server.api.createChildRouter(config.root)
